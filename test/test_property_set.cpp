...
int main() {
    ...
    bool threw = false;
    try {
        ps.apply(json::parse(R"({"gain": 9.0, "buf": 9})"), config_type::RUNTIME);
    ...
    threw = false;
    try {
        ps.apply(json::parse(R"({"gain": -1.0, "channels": {"L": {"bandwidth": 10e6}}})"), config_type::RUNTIME);
    ...
    threw = false;
    try {
        ps.apply(json::parse(R"({"channels": {"X": {"bandwidth": 10e6}}})"), config_type::RUNTIME);
    } catch (const props::validation_error&) {
        threw = true;
    }
    ...
    threw = false;
    try {
        ps.apply(json::parse(R"({"nope": 1})"));
    } catch (const props::unknown_property&) {
        threw = true;
    }
    assert(threw);
    ...
    assert(schema[1]["name"] == "buf" && schema[1]["type"] == "integer" && schema[1].contains("minimum") &&
           schema[1].contains("maximum"));
    assert(schema[2]["name"] == "channels" && schema[2]["type"] == "keyed_collection");
    assert(schema[2]["element"]["type"] == "object" &&
           schema[2]["element"]["fields"]["window"]["choices"] == json::parse(R"(["hann","hamming"])"));
    ...
    // element stays valid (same address) across a batch that adds/modifies others.
    {
        ...
        ps4.add_keyed("ch", ch, config_type::RUNTIME).validate_element([](const std::string&, const ChannelConfig& c) {
            return c.bandwidth > 0;
        });
        ps4.apply(json::parse(R"({"ch": {"L": {"bandwidth": 1e6}}})"), config_type::RUNTIME);
        const ChannelConfig* pL = &ch.at("L");
        ps4.apply(json::parse(R"({"ch": {"C": {"bandwidth": 2e6}, "L": {"bandwidth": 3e6}}})"), config_type::RUNTIME);
        assert(ch.size() == 2);
    ...
    // 400-while-live).
    {
        ...
        int warns = 0;
        std::string warned;
        ps6.set_listener_error_handler([&](const std::string& n, const char*) {
            ++warns;
            warned = n;
        });
        ps6.add("v", v, config_type::RUNTIME).on_change([](const json&) { throw std::runtime_error("listener boom"); });
        bool threw = false;
        try {
            ps6.apply(json::parse(R"({"v": 7})"), config_type::RUNTIME);
        } catch (...) {
            threw = true;
        }
