...
protected:
    auto work(in_t in, timestamp /*ts*/, const composite::metadata& /*md*/) -> out_t override {
        ...
        if (wi >= 0 && wi < 64) { m_hits[static_cast<std::size_t>(wi)].fetch_add(1, std::memory_order_relaxed); }
        auto si = in.as_span();
        ...
        auto so = out.as_span();
        for (std::size_t i = 0; i < si.size(); ++i) { so[i] = si[i] * 2; }
        return out;
    }
    ...
    auto finalize(out_t& out, timestamp /*ts*/, const composite::metadata& /*md*/) -> bool override {
        ...
        for (auto v : so) { if (v != seq * 2) { m_bad.store(true, std::memory_order_relaxed); } }  // value/integrity
        ++m_expect;
...
int main() {
    ...
    if (in == nullptr || !feeder.connect(in)) { std::puts("FAIL: connect"); return 1; }

    pipe->start();
    for (std::int64_t s = 0; s < N; ++s) {
        while (in->is_full()) { std::this_thread::yield(); }  // throttle -> no input drops
        auto b = make_mutable<std::int64_t>(4);
        for (auto& v : b.as_span()) { v = s; }
    ...
    if (pipe->m_bad.load()) { std::puts("FAIL: out-of-order retirement or wrong value"); return 1; }
    ...
    int used = 0;
    for (auto& h : pipe->m_hits) { if (h.load() > 0) { ++used; } }
    if (used < 2) { std::printf("FAIL: only %d worker(s) used (no parallelism)\n", used); return 1; }
    ...
    // ---- stop with work still in flight: must not hang/crash/race ----
    {
        ...
        if (in2 == nullptr || !feed2.connect(in2)) { std::puts("FAIL: connect (B)"); return 1; }
        p2->start();
        for (std::int64_t s = 0; s < 20000; ++s) {
            while (in2->is_full()) { std::this_thread::yield(); }
            auto b = make_mutable<std::int64_t>(4);
            for (auto& v : b.as_span()) { v = s; }
            feed2.send_data(std::move(b), timestamp{});
        }
        p2->stop();  // immediately — workers are still busy, slots in flight
        if (p2->m_bad.load()) { std::puts("FAIL: out-of-order under early stop (B)"); return 1; }
    ...
    // ---- runtime num_workers resize: order + zero loss across live resizes ----
    {
        ...
        if (in3 == nullptr || !feed3.connect(in3)) { std::puts("FAIL: connect (C)"); return 1; }
        p3->start();
        ...
        for (std::int64_t s = 0; s < M; ++s) {
            while (in3->is_full()) { std::this_thread::yield(); }
            auto b = make_mutable<std::int64_t>(4);
            for (auto& v : b.as_span()) { v = s; }
        ...
        p3->stop();
        if (p3->m_bad.load()) { std::puts("FAIL: out-of-order across live resize (C)"); return 1; }
        if (p3->m_finalized.load() != M) {
            std::printf("FAIL: resize lost packets, finalized %lld of %lld (C)\n",
                        (long long)p3->m_finalized.load(), (long long)M);
            return 1;
        }
