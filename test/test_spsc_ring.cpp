...
auto read_seq(const mutable_buffer<std::uint64_t>& b, std::atomic<bool>& bad) -> std::uint64_t {
    ...
    if (span.size() != PAYLOAD) { bad.store(true, std::memory_order_relaxed); }
    for (auto v : span) { if (v != seq) { bad.store(true, std::memory_order_relaxed); } }
    return seq;
}
} // namespace
...
int main() {
    // ---- A) throttled: exact FIFO, zero loss ----
    {
        ...
        if (!out.connect(&in)) { std::puts("FAIL: connect (A)"); return 1; }
        ...
        std::thread cons([&] {
            ...
            while (expect < N) {
                ...
                if (buf.size() == 0) { continue; }       // empty -> retry
                if (read_seq(buf, bad) != expect) { bad.store(true, std::memory_order_relaxed); }
                ++expect;
        ...
        for (std::uint64_t s = 0; s < N; ++s) {
            while (in.is_full()) { std::this_thread::yield(); }  // throttle => no drops
            auto b = make_mutable<std::uint64_t>(PAYLOAD);
            fill(b, s);
            out.send_data(std::move(b), timestamp{});
        }
        cons.join();
        if (bad.load()) { std::puts("FAIL: corruption/torn-read/reorder (A)"); return 1; }
        if (recv.load() != N) {
    ...
    // ---- B) flooded: drops allowed, but FIFO/integrity of survivors holds ----
    {
        ...
        if (!out.connect(&in)) { std::puts("FAIL: connect (B)"); return 1; }
        ...
        std::thread cons([&] {
            ...
            for (;;) {
                auto [buf, ts, md] = in.get_data();
                if (buf.size() == 0) {
                    if (producer_done.load(std::memory_order_acquire) && in.size() == 0) { break; }
                    continue;
                }
                const std::uint64_t seq = read_seq(buf, bad);
                if (!first && seq <= last) { bad.store(true, std::memory_order_relaxed); }  // dup/reorder
                last = seq;
                first = false;
                recv.fetch_add(1, std::memory_order_relaxed);
            }
        ...
        cons.join();
        if (bad.load()) { std::puts("FAIL: reorder/dup/corruption on the drop path (B)"); return 1; }
    ...
    // ---- C) consumer drains via get_batch(): exact FIFO preserved ----
    {
        ...
        output_port<mutable_buffer<std::uint64_t>> out{"o"};
        port_t in{"i", 256};
        if (!out.connect(&in)) { std::puts("FAIL: connect (C)"); return 1; }
        ...
        std::thread cons([&] {
            ...
            while (expect < N) {
                ...
                for (std::size_t i = 0; i < k; ++i) {
                    if (read_seq(std::get<0>(batch[i]), bad) != expect) { bad.store(true, std::memory_order_relaxed); }
                    ++expect;
                    recv.fetch_add(1, std::memory_order_relaxed);
                }
            }
        ...
        cons.join();
        if (bad.load()) { std::puts("FAIL: corruption/reorder via get_batch (C)"); return 1; }
        if (recv.load() != N) { std::puts("FAIL: get_batch lost packets (C)"); return 1; }
    ...
    // ---- D) producer send_batch(): exact FIFO via one-shot batched enqueue ----
    {
        ...
        if (!out.connect(&in)) { std::puts("FAIL: connect (D)"); return 1; }
        ...
        for (std::uint64_t s = 0; s < N;) {
            ...
            while (in.available_capacity() < k) { std::this_thread::yield(); }  // room -> no drops
            out.send_batch(std::span<mutable_buffer<std::uint64_t>>(bufs), timestamp{});
            s += k;
        }
        cons.join();
        if (bad.load()) { std::puts("FAIL: send_batch reorder/corruption (D)"); return 1; }
        if (recv.load() != N) { std::puts("FAIL: send_batch lost packets (D)"); return 1; }
    ...
    //         (Deterministic; producer then consumer, driven from this thread.)
    {
        ...
        if (!out.connect(&in)) { std::puts("FAIL: connect (E)"); return 1; }
        for (std::uint64_t s = 0; s < 4; ++s) {                // fill the ring
            auto b = make_mutable<std::uint64_t>(PAYLOAD); fill(b, s);
            out.send_data(std::move(b), timestamp{});
        }
        ...
        for (std::uint64_t expect = 0; expect < 4; ++expect) { // original 4 intact, in order
            ...
            if (buf.size() != PAYLOAD || buf[0] != expect) {
                std::printf("FAIL (E): got %s seq %llu, expected %llu (unread slot overwritten)\n",
                            buf.size() == 0 ? "empty" : "value",
                            (unsigned long long)(buf.size() ? buf[0] : 0), (unsigned long long)expect);
                return 1;
            }
        }
        auto [tail_buf, tts, tmd] = in.get_data();
        if (tail_buf.size() != 0) { std::puts("FAIL (E): ring not empty after draining 4 (phantom data)"); return 1; }
