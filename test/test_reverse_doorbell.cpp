...
public:
    ...
    auto process() -> retval override {
        if (produced.load(std::memory_order_relaxed) >= target) { return retval::NOOP; }
        if (!out.can_send()) { return retval::AWAIT_OUTPUT; }  // backpressured -> reverse-doorbell idle
...
int main() {
    ...
    std::printf("reverse doorbell: drained %ld/%ld in %lld ms (produced=%ld)\n",
                sink->consumed.load(), N, (long long)dt_ms, src->produced.load());

    src->stop();
    sink->stop();

    if (g_fails == 0) { std::puts("REVERSE DOORBELL OK"); }
