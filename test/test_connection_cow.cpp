...
int main() {
    ...
    std::thread cons_a([&] {
        while (!stop.load(std::memory_order_acquire)) {
            ...
            if (buf.size() != 0) {
                recv_a.fetch_add(1, std::memory_order_relaxed);
            }
        }
        while (a.size() != 0) {
            (void)a.get_data();
    ...
    if (bad.load()) {
        std::puts("FAIL: corruption");
        return 1;
    }
    if (recv_a.load() == 0) {
        std::puts("FAIL: steady sink received nothing");
        return 1;
    }
    std::printf("CONNECTION COW OK: a_recv=%llu churns=%llu introspections=%llu\n", (unsigned long long)recv_a.load(),
                (unsigned long long)churns.load(), (unsigned long long)introspections.load());
    return 0;
}
