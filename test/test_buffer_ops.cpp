...
int main() {
    ...
    auto cap = c.capacity();
    c.clear();
    assert(c.size() == 0 && c.capacity() == cap);
    ...
    assert(threw);
    auto im = std::move(b).to_immutable();
    assert(im.size() == 16 && im[3] == 3.0f);
    ...
    if (fails) {
        std::fprintf(stderr, "%d buffer-safety check(s) FAILED\n", fails);
        return 1;
    }
