...
struct Format { bool is_complex{}; std::string type; std::uint32_t bit_width{}; };
COMPOSITE_STRUCT(Format, is_complex, type, bit_width);

struct ChannelConfig {
    std::string name; double center_freq{}; double bandwidth{};
    std::uint32_t decimation{1}; Window window{Window::hann}; bool enabled{true};
};
...
int main() {
    ...
    ChannelConfig c2; rfl::decode(j, c2);
    assert(rfl::equal(c, c2));

    // 2. partial 7396 merge: only the specified scalar changes
    rfl::merge(c, json::parse(R"({"center_freq": 2.0e9})"));
    assert(c.center_freq == 2.0e9 && c.bandwidth == 20e6 && c.name == "Lband"
           && c.window == Window::blackman_harris);

    // 3. nested struct + optionals round-trip
    SignalOverrides so;
    so.center_frequency = 1.0e9; so.bandwidth = 5e6;
    so.data_format = {true, "ci16", 16}; so.transport = "udp";
    SignalOverrides so2; rfl::decode(rfl::encode(so), so2);
