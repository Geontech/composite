...
struct inner_t { int a{0}; int b{7}; COMPOSITE_FIELDS(inner_t, a, b); };
struct outer_t {
    inner_t sub;
    double scale{1.0};
...
int main() {
    ...
    // ---- attribute-synthesized validators reject bad values; struct untouched ----
    {
        ...
        cp::config_binding<filt> b{c, config_type::INITIALIZE};
        bool threw = false;
        try { b.prepare(json{{"gain", 99.0}}, config_type::RUNTIME); }
        catch (const validation_error& e) { threw = true; check(std::string(e.what()).find("out of range") != std::string::npos, "range reason surfaced"); }
        check(threw && c->gain == 1.0, "range validator rejects, struct intact");

        threw = false;
        try { b.prepare(json{{"taps", 7}}, config_type::RUNTIME); }   // 7 is not a power of two
        catch (const validation_error& e) { threw = true; check(std::string(e.what()).find("power of two") != std::string::npos, "pow2 reason"); }
        check(threw, "power_of_two validator rejects 7");
        b.prepare(json{{"taps", 16}}, config_type::RUNTIME); b.notify(b.commit());
        check(c->taps == 16, "power_of_two accepts 16");

        threw = false;
        try { b.prepare(json{{"mode", "wavelet"}}, config_type::INITIALIZE); }  // not in one_of
        catch (const validation_error&) { threw = true; }
    ...
    // ---- per-field configurability: a RUNTIME patch can't touch an INITIALIZE field ----
    {
        ...
        bool threw = false;
        try { b.prepare(json{{"mode", "iir"}}, config_type::RUNTIME); }
        catch (const config_violation& e) { threw = true; check(e.name == "mode", "violation names the field"); }
        check(threw && c->mode == "fir", "RUNTIME write to INITIALIZE-only field rejected");
        // INITIALIZE context may set it
        b.prepare(json{{"mode", "iir"}}, config_type::INITIALIZE); b.notify(b.commit());
    ...
    // ---- nested-field null-reset via the binding default still works (Phase 0 path) ----
    {
        config<filt> c;
        cp::config_binding<filt> b{c, config_type::INITIALIZE};
        b.prepare(json{{"gain", 5.0}}, config_type::RUNTIME); b.notify(b.commit());
        check(c->gain == 5.0, "gain set to 5");
        b.prepare(json{{"gain", nullptr}}, config_type::RUNTIME); b.notify(b.commit());
    ...
    // ---- 2020-12 schema ----
    {
        ...
        check(s["properties"]["gain"]["minimum"] == 0.0 && s["properties"]["gain"]["maximum"] == 10.0, "gain range -> min/max");
    ...
    if (g_fails != 0) { std::fprintf(stderr, "%d config check(s) FAILED\n", g_fails); return 1; }
