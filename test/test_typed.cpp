...
struct ChannelConfig {
    double center_freq{}; double bandwidth{}; std::uint32_t decimation{1};
    Window window{Window::hann}; bool enabled{true};
};
...
int main() {
    ...
    double gain = 1.0;
    json last; int fires = 0;
    props::typed_property<double> p_gain{"gain", &gain, config_type::RUNTIME};
    p_gain.validate([](const double& g){ return g > 0.0; })
          .on_change([&](const json& d){ last = d; ++fires; });
    ...
    bool threw = false;
    try { p_gain.apply(json(-1.0)); } catch (const props::validation_error&) { threw = true; }
    ...
    threw = false;
    try { p_buf.apply(json(8), config_type::RUNTIME); } catch (const props::config_violation&) { threw = true; }
    assert(threw && buf == 0);
    ...
    chans.validate_element([](const std::string&, const ChannelConfig& c){ return c.bandwidth > 0.0; })
         .validate_list([&](const std::map<std::string, ChannelConfig>& m){
             double tot = 0; for (auto& [k, c] : m) { (void)k; tot += c.bandwidth; }
             return tot <= budget; })
    ...
    threw = false;
    try { chans.apply(json::parse(R"({"Xband": {"center_freq": 6e9, "bandwidth": 10e6}})"), config_type::RUNTIME); }
    ...
    threw = false;
    try { chans.apply(json::parse(R"({"Bad": {"bandwidth": -1.0}})"), config_type::RUNTIME); }
    catch (const props::validation_error&) { threw = true; }
    ...
    // ---- P0b: validate(fn, "reason") surfaces the reason in the rejection ----
    {
        ...
        bool caught = false;
        try { p_rate.apply(json(-5), config_type::RUNTIME); }
