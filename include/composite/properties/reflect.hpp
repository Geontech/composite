...
template <typename T> struct fields_tag {};

template <typename T, typename = void>
struct has_adl_fields : std::false_type {};
template <typename T>
struct has_adl_fields<T, std::void_t<
    decltype(composite_reflect_fields(std::declval<fields_tag<T>>()))>> : std::true_type {};
...
struct range_attr        { double lo; double hi; };       ///< numeric inclusive bounds
struct unit_attr         { std::string_view text; };
struct doc_attr          { std::string_view text; };
struct power_of_two_attr {};
struct runtime_attr      {};                              ///< field is RUNTIME-configurable
template <std::size_t N> struct one_of_attr { std::array<std::string_view, N> values; };
...
template <typename Lo, typename Hi> constexpr auto range(Lo lo, Hi hi) -> range_attr {
    return range_attr{static_cast<double>(lo), static_cast<double>(hi)};
}
constexpr auto unit(std::string_view u) -> unit_attr { return unit_attr{u}; }
constexpr auto doc(std::string_view d) -> doc_attr { return doc_attr{d}; }
inline constexpr power_of_two_attr power_of_two{};
inline constexpr runtime_attr runtime{};
template <typename... S> constexpr auto one_of(S... s) -> one_of_attr<sizeof...(S)> {
    return one_of_attr<sizeof...(S)>{ { std::string_view{s}... } };
}
} // namespace attr

template <typename T> struct is_one_of_attr : std::false_type {};
template <std::size_t N> struct is_one_of_attr<one_of_attr<N>> : std::true_type {};
...
constexpr auto tuple_has_one_of() -> bool {
    ...
    std::apply([&](const auto&... a) {
        ((found = found || is_one_of_attr<std::decay_t<decltype(a)>>::value), ...);
    }, Tup{});
    return found;
}
...
constexpr auto has_attr(const Tup& attrs) -> bool {
    bool found = false;
    std::apply([&](const auto&... a) {
        ((found = found || std::is_same_v<std::decay_t<decltype(a)>, A>), ...);
    }, attrs);
    return found;
}
...
constexpr auto find_attr(const Tup& attrs) -> const A* {
    ...
    std::apply([&](const auto&... a) {
        ([&] { if constexpr (std::is_same_v<std::decay_t<decltype(a)>, A>) { result = &a; } }(), ...);
    }, attrs);
    return result;
}
...
template <typename T> struct is_optional<std::optional<T>> : std::true_type { using inner = T; };
template <typename T> struct is_vector : std::false_type {};
template <typename T> struct is_vector<std::vector<T>> : std::true_type { using elem = T; };
...
auto encode(const M& v) -> json {
    if constexpr (std::is_same_v<M, json>) {
        ...
        json j = json::object();
        std::apply([&](auto&&... f) {
            ((j[std::string(f.name)] = encode(v.*(f.ptr))), ...);
        }, descriptor<M>::fields());
        return j;
...
inline auto suggest(std::string_view unknown, const std::vector<std::string_view>& known)
    -> std::string {
...
struct scoped_path {
    ctx& c;
    std::size_t prev;
    scoped_path(ctx& ctx_, std::string_view seg, bool index = false)
        : c(ctx_), prev(ctx_.path.size()) {
        if (index) { c.path += '['; c.path += seg; c.path += ']'; }
        else { if (!c.path.empty()) { c.path += '.'; } c.path += seg; }
    }
...
template <typename M> auto decode_impl(const json& j, M& out, ctx& c) -> void;
template <typename M> auto merge_impl(M& obj, const json& patch, const M* dflt, ctx& c) -> void;
template <typename M> auto merge_field_impl(M& field, const json& p, const M* dflt, ctx& c) -> void;
...
auto decode_number(const json& j, M& out, ctx& c) -> void {
    if (!j.is_number()) { c.fail("expected a number"); return; }
    if constexpr (std::is_integral_v<M>) {
        if (j.is_number_float()) {
            ...
            double intpart = 0.0;
            if (std::modf(d, &intpart) == 0.0) { c.fail("value out of range " + int_range_msg<M>()); }
            else { c.fail("expected an integer, not a fractional number"); }
            return;
        }
        ...
        if (j.is_number_unsigned()) {
            ...
            if constexpr (std::is_unsigned_v<M>) {
                if (v < 0) { c.fail("expected a non-negative integer, got " + std::to_string(v)); return; }
                if (static_cast<std::uint64_t>(v) > static_cast<std::uint64_t>(lim::max())) {
                    c.fail("value " + std::to_string(v) + " out of range " + int_range_msg<M>());
                    return;
                }
            } else if (v < static_cast<std::int64_t>(lim::min()) ||
                       v > static_cast<std::int64_t>(lim::max())) {
                c.fail("value " + std::to_string(v) + " out of range " + int_range_msg<M>());
                return;
            }
            out = static_cast<M>(v);
...
auto decode_enum(const json& j, E& out, ctx& c) -> void {
    if (!j.is_string()) { c.fail("expected one of the allowed names (as a string)"); return; }
    const auto s = j.template get<std::string>();
    std::string choices;
    for (const auto& [val, name] : enum_traits<E>::names()) {
        if (name == s) { out = val; return; }
...
auto reject_unknown_fields(const json& j, ctx& c) -> void {
    ...
    for (auto it = j.begin(); it != j.end(); ++it) {
        ...
        bool ok = false;
        for (auto k : known) { if (k == key) { ok = true; break; } }
        if (!ok) {
...
auto decode_impl(const json& j, M& out, ctx& c) -> void {
    if constexpr (std::is_same_v<M, json>) {
        ...
        if (!j.is_object()) { c.fail("expected an object"); return; }
        reject_unknown_fields<M>(j, c);
        std::apply([&](auto&&... f) {
            ([&] {
                const std::string key{f.name};
                if (!j.contains(key)) { return; }   // absent: keep current/default
                scoped_path sp{c, f.name};
                decode_impl(j.at(key), out.*(f.ptr), c);
            }(), ...);
        }, descriptor<M>::fields());
    } else if constexpr (is_optional<M>::value) {
        if (j.is_null()) { out.reset(); }
        else { typename is_optional<M>::inner tmp{}; decode_impl(j, tmp, c); out = std::move(tmp); }
    } else if constexpr (is_vector<M>::value) {
        if (!j.is_array()) { c.fail("expected an array"); return; }
        out.clear();
        std::size_t i = 0;
        for (const auto& e : j) {
        ...
        if (!j.is_boolean()) { c.fail("expected a boolean (true/false)"); return; }
        ...
        if (!j.is_string()) { c.fail("expected a string"); return; }
...
auto merge_field_impl(M& field, const json& p, const M* dflt, ctx& c) -> void {
    if constexpr (std::is_same_v<M, json>) {
        ...
        if (p.is_null()) { field.reset(); }  // optional's "default" is nullopt
        else {
            if (!field.has_value()) { field.emplace(); }
            const typename is_optional<M>::inner* idflt =
                (dflt != nullptr && dflt->has_value()) ? &**dflt : nullptr;
            merge_field_impl(*field, p, idflt, c);
        }
    } else if constexpr (is_reflected_v<M>) {
        if (p.is_null()) { field = (dflt != nullptr) ? *dflt : M{}; }
        else { merge_impl(field, p, dflt, c); }
    } else {
        if (p.is_null()) { field = (dflt != nullptr) ? *dflt : M{}; }  // reset to registered/typed default
        else { decode_impl(p, field, c); }                            // scalar/enum/array: strict replace
    }
}
...
auto merge_impl(M& obj, const json& patch, const M* dflt, ctx& c) -> void {
    if constexpr (is_reflected_v<M>) {
        if (!patch.is_object()) { c.fail("expected an object"); return; }
...
auto decode(const json& j, M& out, std::string_view root = {}) -> void {
    ...
    if (!c.errors.empty()) { throw decode_failure(std::move(c.errors)); }
}
...
auto equal(const M& a, const M& b) -> bool {
    if constexpr (is_reflected_v<M>) {
        bool eq = true;
        std::apply([&](auto&&... f) { ((eq = eq && equal(a.*(f.ptr), b.*(f.ptr))), ...); },
                   descriptor<M>::fields());
        return eq;
    } else {
        return a == b;
...
template <typename M> auto diff_value(const M& a, const M& b) -> json;
...
template <typename M> auto type_schema() -> json;
...
auto type_schema() -> json {
    ...
    if constexpr (std::is_same_v<M, json>) {
        ...
        std::apply([&](auto&&... f) {
            ((fields[std::string(f.name)] =
                  type_schema<typename std::decay_t<decltype(f)>::member_type>()), ...);
        }, descriptor<M>::fields());
        ...
        for (const auto& [val, name] : enum_traits<M>::names()) { (void)val; choices.push_back(std::string(name)); }
...
#define COMPOSITE_RFL_ENUMV(E, v) ::std::pair<E, ::std::string_view>{E::v, #v}
...
#define COMPOSITE_RFL_GET(_1,_2,_3,_4,_5,_6,_7,_8,_9,_10,_11,_12,_13,_14,_15,_16,_17,_18,_19,_20,_21,_22,_23,_24,_25,_26,_27,_28,_29,_30,_31,_32,_33,_34,_35,_36,_37,_38,_39,_40,_41,_42,_43,_44,_45,_46,_47,_48,_49,_50,_51,_52,_53,_54,_55,_56,_57,_58,_59,_60,_61,_62,_63,_64,N,...) N
#define COMPOSITE_RFL_FE(M, c, ...) \
    COMPOSITE_RFL_EXPAND(COMPOSITE_RFL_GET(__VA_ARGS__, \
        COMPOSITE_RFL_FE_64, COMPOSITE_RFL_FE_63, COMPOSITE_RFL_FE_62, COMPOSITE_RFL_FE_61, COMPOSITE_RFL_FE_60, COMPOSITE_RFL_FE_59, \
        COMPOSITE_RFL_FE_58, COMPOSITE_RFL_FE_57, COMPOSITE_RFL_FE_56, COMPOSITE_RFL_FE_55, COMPOSITE_RFL_FE_54, COMPOSITE_RFL_FE_53, \
        COMPOSITE_RFL_FE_52, COMPOSITE_RFL_FE_51, COMPOSITE_RFL_FE_50, COMPOSITE_RFL_FE_49, COMPOSITE_RFL_FE_48, COMPOSITE_RFL_FE_47, \
        COMPOSITE_RFL_FE_46, COMPOSITE_RFL_FE_45, COMPOSITE_RFL_FE_44, COMPOSITE_RFL_FE_43, COMPOSITE_RFL_FE_42, COMPOSITE_RFL_FE_41, \
        COMPOSITE_RFL_FE_40, COMPOSITE_RFL_FE_39, COMPOSITE_RFL_FE_38, COMPOSITE_RFL_FE_37, COMPOSITE_RFL_FE_36, COMPOSITE_RFL_FE_35, \
        COMPOSITE_RFL_FE_34, COMPOSITE_RFL_FE_33, COMPOSITE_RFL_FE_32, COMPOSITE_RFL_FE_31, COMPOSITE_RFL_FE_30, COMPOSITE_RFL_FE_29, \
        COMPOSITE_RFL_FE_28, COMPOSITE_RFL_FE_27, COMPOSITE_RFL_FE_26, COMPOSITE_RFL_FE_25, COMPOSITE_RFL_FE_24, COMPOSITE_RFL_FE_23, \
        COMPOSITE_RFL_FE_22, COMPOSITE_RFL_FE_21, COMPOSITE_RFL_FE_20, COMPOSITE_RFL_FE_19, COMPOSITE_RFL_FE_18, COMPOSITE_RFL_FE_17, \
        COMPOSITE_RFL_FE_16, COMPOSITE_RFL_FE_15, COMPOSITE_RFL_FE_14, COMPOSITE_RFL_FE_13, COMPOSITE_RFL_FE_12, COMPOSITE_RFL_FE_11, \
        COMPOSITE_RFL_FE_10, COMPOSITE_RFL_FE_9, COMPOSITE_RFL_FE_8, COMPOSITE_RFL_FE_7, COMPOSITE_RFL_FE_6, COMPOSITE_RFL_FE_5, \
        COMPOSITE_RFL_FE_4, COMPOSITE_RFL_FE_3, COMPOSITE_RFL_FE_2, COMPOSITE_RFL_FE_1, \
        COMPOSITE_RFL_FE_0_UNUSED)(M, c, __VA_ARGS__))

#define COMPOSITE_STRUCT(T, ...)                                                     \
    template <> struct composite::reflect::descriptor<T, void> {                     \
...
#define COMPOSITE_ENUM(E, ...)                                                       \
    template <> struct composite::reflect::enum_traits<E> {                          \
...
#define COMPOSITE_RFL_FLD_1(T, tok)                                                  \
    ::composite::reflect::fld(COMPOSITE_RFL_STRHEAD tok, &T::COMPOSITE_RFL_HEAD tok)  \
        .with(COMPOSITE_RFL_TAIL tok)
