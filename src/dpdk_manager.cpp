...
auto manager::initialize(const dpdk::config& config) -> bool {
    ...
    for (const auto& port_config : config.ports) {
        if (!configure_port(port_config)) {
            spdlog::error("Failed to configure DPDK port {} ({})", port_config.port_id, port_config.interface);
            cleanup_partial();
            return false;
        }

        spdlog::info("Configured DPDK port {} (interface={}, rx_queues={}, mempool={})", port_config.port_id,
                     port_config.interface, port_config.rx_queues, port_config.mempool_name);
    }
...
auto manager::list_available_ports() const -> std::vector<port_summary> {
    ...
    for (uint16_t port_id = 0; port_id < nb_ports; ++port_id) {
        ...
        if (ret != 0) {
            spdlog::warn("Failed to get device info for port {}: {}", port_id, rte_strerror(-ret));
            continue;
        }
...
auto manager::configure_port(const port_config& config) -> bool {
    ...
    if (m_registry.is_port_configured(port_id) || m_registry.get_port_id_for_interface(config.interface).has_value()) {
        spdlog::error("Port {} / interface '{}' already configured; refusing to reconfigure", port_id,
                      config.interface);
        return false;
    }
    ...
    int ret = rte_eth_dev_info_get(port_id, &dev_info);
    if (ret != 0) {
        spdlog::error("Failed to get device info for port {}: {}", port_id, rte_strerror(-ret));
        return false;
    }

    // Log device information
    spdlog::debug("Port {}: driver={}, max_rx_queues={}, max_tx_queues={}", port_id, dev_info.driver_name,
                  dev_info.max_rx_queues, dev_info.max_tx_queues);

    // Validate queue counts
    if (config.rx_queues > dev_info.max_rx_queues) {
        spdlog::error("Port {} requested {} RX queues but only {} supported", port_id, config.rx_queues,
                      dev_info.max_rx_queues);
        return false;
    }

    if (config.tx_queues > dev_info.max_tx_queues) {
        spdlog::error("Port {} requested {} TX queues but only {} supported", port_id, config.tx_queues,
    ...
    for (uint16_t q = 0; q < config.rx_queues; q++) {
        ret = rte_eth_rx_queue_setup(port_id, q, config.rx_descriptors, rte_eth_dev_socket_id(port_id),
                                     nullptr, // Use driver default RX config
                                     mempool);

        if (ret != 0) {
            spdlog::error("Failed to setup RX queue {} on port {}: {}", q, port_id, rte_strerror(-ret));
            return false;
        }
    }
    ...
    for (uint16_t q = 0; q < config.tx_queues; q++) {
        ret = rte_eth_tx_queue_setup(port_id, q, config.tx_descriptors, rte_eth_dev_socket_id(port_id),
                                     nullptr // Use default TX config
        );

        if (ret != 0) {
            spdlog::error("Failed to setup TX queue {} on port {}: {}", q, port_id, rte_strerror(-ret));
            return false;
        }
    }
    ...
    if (!m_registry.register_port(config.interface, port_id, config.rx_queues, mempool)) {
        spdlog::error("Port {} / interface '{}' already configured; refusing to overwrite", port_id, config.interface);
...
auto manager::create_mempool(const port_config& config) -> rte_mempool* {
    ...
    if (existing) {
        ...
        if (it != m_mempool_configs.end()) {
            ...
            if (existing_cfg.mempool_size != config.mempool_size ||
                ...
                spdlog::warn("Mempool '{}' already exists with different settings "
                             "(size={}, cache={}, room={}); requested size={}, cache={}, room={}",
                             config.mempool_name, existing_cfg.mempool_size, existing_cfg.mempool_cache_size,
                             existing_cfg.mbuf_data_room_size, config.mempool_size, config.mempool_cache_size,
                             config.mbuf_data_room_size);
            }
        } else {
            spdlog::warn("Reusing existing mempool '{}' without known config; "
                         "settings are not verified",
                         config.mempool_name);
            m_mempool_configs[config.mempool_name] = {config.mempool_size, config.mempool_cache_size,
                                                      config.mbuf_data_room_size};
        }
    ...
    // Create new mempool
    rte_mempool* pool =
        rte_pktmbuf_pool_create(config.mempool_name.c_str(), config.mempool_size, config.mempool_cache_size,
                                0, // private data size
                                config.mbuf_data_room_size, rte_socket_id());

    if (!pool) {
        spdlog::error("Failed to create mempool '{}': {}", config.mempool_name, rte_strerror(rte_errno));
        return nullptr;
    }
    ...
    m_mempool_configs[config.mempool_name] = {config.mempool_size, config.mempool_cache_size,
                                              config.mbuf_data_room_size};
    spdlog::debug("Created mempool '{}' with {} mbufs (data_room={})", config.mempool_name, config.mempool_size,
                  config.mbuf_data_room_size);
...
auto manager::get_port_id_for_interface(const std::string& interface_name) const -> std::optional<uint16_t> {
...
auto manager::is_queue_available(const std::string& interface_name, uint16_t queue_id) const -> bool {
...
auto manager::allocate_next_available_queue(const std::string& interface_name) -> std::optional<uint16_t> {
    if (!m_initialized.load(std::memory_order_acquire)) {
        spdlog::warn("Refusing queue allocation on '{}': DPDK not initialized", interface_name);
        return std::nullopt;
    }
    ...
    if (!q) {
        spdlog::error("No available queue on interface '{}' "
                      "(unknown interface or all queues allocated)",
                      interface_name);
        return std::nullopt;
    }
...
auto manager::release_queue(const std::string& interface_name, uint16_t queue_id) -> void {
    if (!m_registry.release_queue(interface_name, queue_id)) {
        spdlog::warn("Attempted to release queue {} on interface '{}' that was not "
                     "allocated (or interface/queue invalid)",
                     queue_id, interface_name);
        return;
    }
